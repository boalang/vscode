...
export class StudyConfigJSONLinkProvider implements vscode.DocumentLinkProvider {
	provideDocumentLinks(document: vscode.TextDocument, token: vscode.CancellationToken): vscode.DocumentLink[] | undefined {
        ...
        for (const output of document.getText().matchAll(/"query":\s*"([^"]+\.boa)"/g)) {
            const startPos = output.index + output[0].length - output[1].length - 1;
            const range = new vscode.Range(document.positionAt(startPos), document.positionAt(startPos + output[1].length));
            const link = new vscode.DocumentLink(range, vscode.Uri.parse(`file://${jsonPath}/${scriptPath}/${output[1]}`));
            link.tooltip = `View the Boa query '${scriptPath}/${output[1]}'`;
            links.push(link);
        }

        // looks for CSV files, e.g.: "csv": "kotlin/dupes.csv"
        for (const output of document.getText().matchAll(/"([^"]+\.csv)"/g)) {
            const range = new vscode.Range(document.positionAt(output.index + 1), document.positionAt(output.index + 1 + output[1].length));
            const link = new vscode.DocumentLink(range, vscode.Uri.parse(`file://${jsonPath}/${csvPath}/${output[1]}`));
            link.tooltip = `View the Boa output CSV file '${csvPath}/${output[1]}'`;
            links.push(link);
        }

        // looks for replacement files, e.g.: "file": "escape.boa"
        for (const output of document.getText().matchAll(/"file":\s*"([^"]+\.boa)"/g)) {
            const startPos = output.index + output[0].length - output[1].length - 1;
            const range = new vscode.Range(document.positionAt(startPos), document.positionAt(startPos + output[1].length));
            const link = new vscode.DocumentLink(range, vscode.Uri.parse(`file://${jsonPath}/${snippetPath}/${output[1]}`));
            link.tooltip = `View the Boa query output file '${snippetPath}/${output[1]}'`;
            links.push(link);
        }
